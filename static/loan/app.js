...
doc.ready(function () {
  ...
  $('.btn-primary').on('click', function (e) {
    ...
    var rent_change = $('#rent_change').val();
    ...
    for (var i = 0; i <= loan_years; i++) {
      ...
      if (i > 0 && remaining_debt > 0) {
        if (prepayments > remaining_debt) {
          prepayments = remaining_debt;
        }
        remaining_debt -= prepayments;
      }
      if (remaining_debt < 0) {
        remaining_debt = 0;
      }
      ...
      tr.append($('<td class="text-xs-right"/>').text(rent * month_count - (management_fee + administrative_expense + reserve_fund_for_repair) - repayment - tax));
      tr.append($('<td class="text-xs-right"/>').text(i > 0 && remaining_debt > 0 ? prepayments : 0));
      tbody.append(tr);
      year++;
    }
  });
